...
def local_rbf_kernel(X, y, gamma=None):
    # todo make final representation sparse (optional)
    assert len(X.shape) == 2
    assert len(y.shape) == 1
    assert torch.all(
        y == y.sort()[0]
    ), "This function assumes the dataset is sorted by y"
...
def change_gamma(K, old_gamma, new_gamma):
...
if __name__ == "__main__":
    ...
    assert torch.allclose(
        rbf_kernel(test_X), torch.from_numpy(rbf_kernel_sklearn(test_X.numpy()))
    )
    print("Testing gamma=0.026")
    assert torch.allclose(
        rbf_kernel(test_X, gamma=0.026),
        torch.from_numpy(rbf_kernel_sklearn(test_X.numpy(), gamma=0.026)),
    )
