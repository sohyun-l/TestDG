...
class testdg_cifar100c(nn.Module):
    ...
    def forward(self, x, c_iter, pre_conf, domain_change, i_c, conf_diff, domain_label, domain_memory, prototypes, clean):
        if clean == True:
            with torch.no_grad():
                self.model_ema.eval()
                self.model.eval()
            ...
            for i in range(self.steps):
                if domain_change == True:
                    domain_label += 1
                outputs, conf, domain_memory, prototypes = self.forward_and_adapt(x, self.model, self.optimizer, c_iter, domain_label, domain_change, domain_memory, prototypes)
               

        return outputs, conf, domain_label, domain_memory, prototypes
    ...
    @torch.enable_grad()  
    def forward_and_adapt(self, x, model, optimizer, i_c, domain_label, domain_change, domain_memory, prototypes):
        self.model_ema.eval()
        ...
        if i_c % 50 < 20:
            ...
            if domain_label == 0:
                ...
                labels_ori = torch.ones(50, dtype=torch.float).cuda()
                labels_aug = torch.zeros(50, dtype=torch.float).cuda()
            ...
            if domain_label != 0:
                ...
                prototypes_new = nn.Parameter(torch.randn(50, 4, requires_grad=True, device="cuda"))
                for i in range(3):
                    chamfer_dist_pre = chamfer_dist(pair_ori.detach().unsqueeze(0), prototypes.detach().unsqueeze(0))
                    outputs = self.model(x.detach()).detach()
                    optimizer_proto = torch.optim.Adam([prototypes_new], lr=0.008)  
        ...
        return standard_ema, conf, self.domain_memory, prototypes
    ...
    def generate_representation(self, feat, rbf_gamma=None, sample_num=1, std_mul=0.0):
        ...
        selected = select_prototypes(kernel_matrix, num_prototypes=50)
