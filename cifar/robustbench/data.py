...
def _load_dataset_name(
    ...
    for i, (x, y) in enumerate(test_loader):
        ...
        paths.extend([f"{start_idx + j}" for j in range(x.size(0))])
        if n_examples is not None and end_idx >= n_examples:
            break
    ...
    return x_test_tensor, y_test_tensor, paths
...
def load_cifar10_name(
    ...
    dataset = datasets.CIFAR10(root=data_dir,
                               ...
                               transform=transforms_test,
                               download=True)
    return _load_dataset_name(dataset, n_examples)
...
def load_imagenet(
    ...
    transforms_test = PREPROCESSINGS[prepr]
    imagenet = CustomImageFolder(data_dir + '/train', transforms_test)
...
_clean_dataset_loaders: Dict[BenchmarkDataset, CleanDatasetLoader] = {
    ...
    BenchmarkDataset.cifar_10: load_cifar10_name,
...
def load_cifar10c_name(
    ...
    return load_corruptions_cifar_name(BenchmarkDataset.cifar_10, n_examples,
                                  severity, data_dir, corruptions, shuffle)
...
def load_corruptions_cifar_name(
        dataset: BenchmarkDataset,
        n_examples: int,
        severity: int,
        data_dir: str,
        corruptions: Sequence[str] = CORRUPTIONS,
        shuffle: bool = False) -> Tuple[torch.Tensor, torch.Tensor, list]:
    assert 1 <= severity <= 5
    n_total_cifar = 10000
    ...
    x_test_list, y_test_list, paths_list = [], [], []
    n_pert = len(corruptions)
    for corruption in corruptions:
        corruption_file_path = data_root_dir / (corruption + '.npy')
        if not corruption_file_path.is_file():
        ...
        for idx in range(n_img):
            paths_list.append(str(corruption_file_path) + f'_{idx}')

    x_test, y_test = np.concatenate(x_test_list), np.concatenate(y_test_list)
    if shuffle:
        rand_idx = np.random.permutation(np.arange(len(x_test)))
        x_test, y_test = x_test[rand_idx], y_test[rand_idx]
        paths_list = np.array(paths_list)[rand_idx].tolist()
    ...
    x_test = torch.tensor(x_test)[:n_examples]
    y_test = torch.tensor(y_test)[:n_examples]
    paths_list = paths_list[:n_examples]
