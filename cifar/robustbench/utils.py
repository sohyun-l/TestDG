    ...
    with torch.no_grad():
        for counter in range(n_batches):
            ...
            y_curr = y[counter * batch_size:(counter + 1) *
                       batch_size].to(device)
            # output = model(x_curr)
            output = model(x_curr, pre_iter+counter, clean=False)
            acc_iter = (output.max(1)[1] == y_curr).float().sum().item() / batch_size
            err_iter = 1. - acc_iter
            acc_list.append(err_iter)
            # if 
            acc += (output.max(1)[1] == y_curr).float().sum()

    return acc.item() / x.shape[0], n_batches, acc_list


def accuracy_cifar100c(model: nn.Module,
                   x: torch.Tensor,
                   y: torch.Tensor,
                   batch_size: int = 100,
                   pre_iter: int = 0,
                   pre_conf: list = [0.0, 0.0],
                   i_c: int = 0,
                   domain_label: int= 0,
                   conf_diff: float = 0.0,
                   domain_memory: list = [],
    ...
    with torch.no_grad():
        for counter in range(n_batches):
            ...
            y_curr = y[counter * batch_size:(counter + 1) *
                       batch_size].to(device)
            output, conf, domain_label, domain_memory, prototypes = model(x_curr, pre_iter+counter, pre_conf, domain_change, i_c, conf_diff, domain_label, domain_memory, prototypes, clean=False)
            # if 
            if pre_conf[0]!= 0:
                conf_diff = abs(conf - pre_conf[1])
                if conf_diff < abs(conf - pre_conf[0]):
                    conf_diff = abs(conf - pre_conf[0])
                if conf_diff > 0.16:
                    domain_change = True
                else:
                    domain_change = False
            pre_conf[0] = pre_conf[1]
            pre_conf[1] = conf
            acc += (output.max(1)[1] == y_curr).float().sum()

    return acc.item() / x.shape[0], n_batches, pre_conf, domain_label, conf_diff, domain_memory, prototypes


def accuracy_cifar10c(model: nn.Module,
                   x: torch.Tensor,
                   y: torch.Tensor,
                   acc_list: list,
    ...
    with torch.no_grad():
        for counter in range(n_batches):
            ...
            y_curr = y[counter * batch_size:(counter + 1) *
                       batch_size].to(device)
            output, conf, domain_label, domain_memory, prototypes, random_vector = model(x_curr, pre_iter+counter, pre_conf, domain_change, i_c, conf_diff, domain_label, domain_memory, prototypes, random_vector, clean=False)
    ...
    return acc.item() / x.shape[0], n_batches, pre_conf, domain_label, conf_diff, domain_memory, prototypes, random_vector, acc_list
