...
class AMPLIFIERInjectedLinearReturn(nn.Module):
    def __init__(self, in_features, out_features, bias=False, r=4, r2 = 128):
        super().__init__()
...
def inject_trainable_amplifier_return(
    ...
    """
    inject amplifier into model, and returns amplifier parameter groups.
    """
        ...
        if _module.__class__.__name__ in target_replace_module:
            ...
            for name, _child_module in _module.named_modules():
                if _child_module.__class__.__name__ == "Linear":
                    ...
                    #     list(_module._modules[name].amplifier_up.parameters())
                    # )
                    # require_grad_params.extend(
                    #     list(_module._modules[name].amplifier_down.parameters())
                    # )
                    # _module._modules[name].amplifier_up.weight.requires_grad = True
                    # _module._modules[name].amplifier_down.weight.requires_grad = True
