...
import PIL
from cotta_transforms import get_tta_transforms
...
def update_ema_variables(model_ema, model, alpha_teacher, alpha_amplifier):#, iteration):

    for ema_param, (name, param) in zip(model_ema.parameters(), model.named_parameters()):
...
class testdg(nn.Module):
    def __init__(self, model, optimizer, steps=1, episodic=False, ema=0.99, ema_amplifier = 0.99, rst_m=0.1, unc_thr = 0.2, mse_weight = 0.1, proj_lr=0.001, cls_weight = 1.0, cls_lr=0.001):
        super().__init__()
        self.model = model
    ...
    def forward_and_adapt(self, x, model, optimizer, i_c, domain_label, domain_change, domain_memory, prototypes, random_vector):
        ...
        if i_c % 50 < 20:
            ...
            if domain_label == 0:
                ...
                labels_ori = torch.ones(batch_size, dtype=torch.float).cuda()
                labels_aug = torch.zeros(batch_size, dtype=torch.float).cuda()
            ...
            with torch.no_grad():
                domain_acc = self.compute_domain_accuracy(cls_ori, cls_aug, labels_ori, labels_aug)
            ...
            if domain_label != 0:
                ...
                prototypes_new = nn.Parameter(torch.randn(batch_size, 4, requires_grad=True, device="cuda"))
        ...
        self.model_ema = update_ema_variables(model_ema = self.model_ema, model = self.model, alpha_teacher= self.alpha_teacher, alpha_amplifier = self.alpha_amplifier)


        return standard_ema, conf, self.domain_memory, prototypes, random_vector

    def compute_domain_accuracy(self, cls_ori, cls_aug, labels_ori, labels_aug):
        pred_ori = (cls_ori >= 0).float()
        pred_aug = (cls_aug >= 0).float()
        correct_ori = (pred_ori.squeeze() == labels_ori).float().sum()
        correct_aug = (pred_aug.squeeze() == labels_aug).float().sum()
        total = labels_ori.size(0) + labels_aug.size(0)
        accuracy = (correct_ori + correct_aug) / total
    ...
    def generate_representation(self, feat, rbf_gamma=None, sample_num=1, std_mul=0.0):
        ...
        selected = select_prototypes(kernel_matrix, num_prototypes=batch_size)
...
def copy_model_and_optimizer(model, optimizer):
    ...
    for param in model_ema.parameters():
        param.detach_()
    return model_state, optimizer_state, model_ema, model_anchor
...
def configure_model(model, cfg):
    ...
    if cfg.TEST.ckpt!=None:
        ...
        for k,v in checkpoint['model'].items():
            name = k.replace('model.module.','') # remove `module.`
