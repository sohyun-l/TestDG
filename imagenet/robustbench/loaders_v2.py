...
class CustomDatasetFolder(VisionDataset):
    ...
    def __init__(self, root, loader, extensions=None, transform=None, target_transform=None, is_valid_file=None, train=False):
        ...
        if train == True:
            samples = make_custom_dataset(self.root, 'robustbench/data/imagenet_train_image_ids.txt',
                                          'robustbench/data/imagenet_class_to_id_map.json')
            # print('index:', index)
            # print('index_large:', index_large)
        else:
...
#                                                 is_valid_file=is_valid_file, train=train)
                                          
#         self.imgs = self.samples


class CustomImageFolderV2(CustomDatasetFolder):
    def __init__(self, root, transform=None, target_transform=None,
                 loader=default_loader, is_valid_file=None, train=False,
                 start_idx=0, end_idx=None):
        super(CustomImageFolderV2, self).__init__(root, loader, IMG_EXTENSIONS if is_valid_file is None else None,
                                                transform=transform,
                                                target_transform=target_transform,
                                                is_valid_file=is_valid_file, train=train)
...
if __name__ == '__main__':
    ...
    imagenet = CustomImageFolder_v2(data_dir, transforms.Compose([
